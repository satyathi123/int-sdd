...
# CRITICAL RULE — INT CONTROL PLANE (DYNAMIC DUAL COPY & SYNC)
...
You MUST dynamically copy the entire contents of this directory into BOTH project root control directories:
1. `.agent/`
2. `.agents/`

## Dynamic Copying Protocol:

- **Do NOT rely on hardcoded file lists.** The agent MUST dynamically discover and copy ALL files and subdirectories present in `skills/int-project-setup/resources/INT-Control-Plane/.agent/` directly to BOTH `.agent/` and `.agents/` in the project workspace root.
- **Dynamic Rule Synchronization**: Every rule file (e.g., `.md`, `.agentignore`) present inside `skills/int-project-setup/resources/INT-Control-Plane/.agent/rules/` (including any new rules added now or in the future) MUST be dynamically discovered and copied to BOTH `.agent/rules/` and `.agents/rules/`.
- **Dynamic Workflow Synchronization**: Every workflow file (`.md`) present inside `skills/int-project-setup/resources/INT-Control-Plane/.agent/workflows/` (including `int-project-resume.md`, `int-project-from-brd.md`, `int-brd-ingestion.md`, `int-pr-gate-workflow.md`, etc.) MUST be dynamically discovered and copied to `.agent/workflows/`, `.agents/workflows/`, and `workflows/` in the project workspace root.
- If new rule or workflow files are added to `skills/int-project-setup/resources/INT-Control-Plane/.agent/`, the setup/continuation process MUST dynamically include them automatically across `.agent/`, `.agents/`, and `workflows/`.
...
3. **Mandatory Workflow, Skill & Governance Resolution Hierarchy**:
   ...
   - **Priority 1 (Local Repository First)**: First check if local project workflows (`.agent/workflows/<workflow_name>.md`, `.agents/workflows/<workflow_name>.md`, or `workflows/<workflow_name>.md`), `AGENTS.md`, or local project skills (`.agents/skills/<skill_name>/SKILL.md`) exist inside the project repository root. If present, load and execute the **local project workflows, rules, and skills** first.
   - **Priority 2 (Global Fallback Second)**: If and ONLY if a requested workflow, skill, or rule file is not present locally in the project repository root, fall back to checking global workflows (`<global-config-root>/workflows/`) or global skills (`<global-skills-root>/<skill_name>/SKILL.md`).

This ensures that every team member or AI assistant (Gemini, Claude, Cursor, Windsurf, Copilot, etc.) working on the project prioritizes repository-local workflows and skills directly inside the project folder without relying on external or cloud AI configurations.
